...
The target produit is a living spec: for each story, the agentic notes may point to the target's equivalent screen or flow — the reference implementation already runs in production. Stay inside the PRD scope: nothing from the excluded becomes a story.
...
2. For each story, write verifiable acceptance criteria (each one must be able to become a test), the agentic notes useful for execution, and a complexity score (1-5, same scale as the PRD scope) — a 5 gets split now, not at planning.
...
End with: "Stories ready in docs/stories.md. Next step: /aw-stories-review"
