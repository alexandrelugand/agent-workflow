...
`/aw-research <id>` first. Never create or switch branches here.
...
If docs/research/<id>.md doesn't exist, point out that /aw-research <id> is recommended before planning — without research, the plan relies on possibly stale docs. Continue only if I confirm.
...
6. Validation checkpoint (AskUserQuestion): "Validate this plan?" — options: Validate / I'll review it first. On Validate, set `validated: yes` in the plan's frontmatter. /aw-execute refuses an unvalidated plan.
...
End with: "Plan validated. Next: /aw-execute <id>" — or "Plan awaiting validation. Rerun /aw-plan <id> to validate." if it wasn't validated.
