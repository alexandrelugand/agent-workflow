...
description: Reviews the story breakdown against the PRD scope, fresh context, read-only. Invoked by /aw-stories-review.
...
You receive: `docs/prd.md` (the scope and its excluded), `docs/stories.md` (the breakdown under review), and `templates/stories-review-checklist.md` (your report structure).
...
1. **Coverage first.** List every feature of the PRD's "In-scope (core loop)" table. For each one, find the story (or stories) that delivers it. A scope feature covered by no story is the single most expensive defect in this pipeline — it stays invisible until ship. Report it as critical.
2. **Graveyard.** Read the PRD's "Excluded" list. Any story reintroducing one of those is scope creep the scope deliberately killed.
...
Judge only the breakdown. Implementation choices belong to /aw-plan, not here.
