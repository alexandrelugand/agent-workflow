...
description: Breaks a need or a PRD down into user stories an agent can execute. Use during the Stories phase of the agent-workflow pipeline.
---
...
- Complexity score: rate each story 1-5 (same scale as the PRD scope). A 4 must call out its risk in the agentic notes; a 5 never stays one story — split it before it reaches /aw-plan.
- Target as reference: when the PRD names a target project, point each story to the target's equivalent flow or screen in the agentic notes — the reference implementation provides guidance. Never turn anything from the PRD's excluded (explicitly not developed) into a story.
